...
def main() -> None:
    ...
    if want_github:
        token = (args.github_token or os.getenv('GITHUB_TOKEN') or '').strip()
        username = (args.github_username or os.getenv('GITHUB_USERNAME') or '').strip()
        if not token:
            raise SystemExit('Error: GitHub source enabled but no token found (set GITHUB_TOKEN or --github-token).')
        if not username:
            raise SystemExit('Error: GitHub source enabled but no username found (set GITHUB_USERNAME or --github-username).')

        gh = GitHubAnalytics(token, username, enable_rate_limiting=not args.disable_rate_limiting)
